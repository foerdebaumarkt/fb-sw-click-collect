...
class Migration1761266003NormalizeStaffRecipient extends MigrationStep
{
    public function getCreationTimestamp(): int
    {
        // 2025-11-01 00:00:03 UTC
        return 1761266003;
    }
    ...
    public function update(Connection $connection): void
    {
        ...
        if (($recipient['type'] ?? null) !== 'custom') {
            return;
        }

        $recipientData = $recipient['data'] ?? null;
        if (!\is_array($recipientData)) {
            return;
        }

        $current = $this->normalizeRecipientData($recipientData);
        ...
        if ($storeEmail !== null && $storeEmail !== '') {
            $resolvedName = $storeName !== null && $storeName !== ''
                ? $storeName
                : $this->extractRecipientName($current);
        ...
        if ($target === $current && !$this->isList($recipientData)) {
            return;
        }
